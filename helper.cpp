...
//					std::string tradeIndex = tradeValueTable[(*tradeMap)[currentName]][6]; // Use arrow operator to access the map
...
//		std::cout << "[LOAD] team " << team.teamName << " has been loaded. \n";
//	}
//	return teams;
//}
...
void realTradeAnalyzer(std::vector<Team>& teams, std::string& userId, HANDLE& hConsole) {
	...
	for (const auto& team : teams) {
		if (team.owner_id == userId) {
			...
			for (int i = 0; i < team.roster.size(); i++) {
				...
				std::cout << "1. " << player->name << " " << "TRADE VALUE: " << player->tradeValue << "\n";
			}
	...
	while (selection != 0) {
		...
		std::cin >> selection;
		if (selection == 0) break;
		team1.push_back(tradePartner.roster[selection - 1]);
